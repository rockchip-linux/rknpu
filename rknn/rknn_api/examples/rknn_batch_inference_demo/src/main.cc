...
int main(int argc, char **argv)
{
    ...
    const int MODEL_IN_BATCHSIZE = 4;
    ...
    // Post Process
    for (int n = 0; n < MODEL_IN_BATCHSIZE; ++n)
    {
        for (int i = 0; i < io_num.n_output; i++)
        {
            uint32_t MaxClass[5];
            float fMaxProb[5];
            float *buffer_f32 = (float *)outputs[i].buf;
            float *cur_batch_buf = buffer_f32 + 1001 * n;
            uint32_t cur_batch_sz = outputs[i].size / (4 * MODEL_IN_BATCHSIZE);
            rknn_GetTop(cur_batch_buf, fMaxProb, MaxClass, cur_batch_sz, 5);
